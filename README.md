...
The bot fetches real-time market data, reads macro-economic news headlines, sends everything to an AI model for analysis, and executes the AI's trading decision on a bar-aligned schedule. It includes **4 selectable trading strategies**, a **web dashboard** with live charts, **Telegram remote control** (7 interactive commands), **news API caching**, **market-closed detection**, an optional **economic calendar window**, a **centralized risk gate manager**, a **rule-based trade memory layer**, **API cost controls**, and comprehensive safety controls.
...
14. [Live Trading vs Demo Trading](#live-trading-vs-demo-trading)
15. [Risk Controls](#risk-controls)
...
8. `SPREAD_SPIKE_ALERT_POINTS` sets the spread level where the fast watchdog raises a warning, even before the slower decision loop runs again.
9. `SPREAD_SPIKE_BLOCK_SECONDS` controls how long new BUY and SELL entries stay blocked after the watchdog detects a spread spike, even if the spread normalizes quickly.
10. `M5_CANDLE_COUNT` controls how much 5-minute history is loaded when a strategy uses M5 as its primary decision timeframe.
11. `SCALPING_LOOP_SLEEP_SECONDS` controls the bar-aligned decision cadence for scalping, with the default set to 300 seconds so the bot evaluates on each new M5 bar.
...
2. The other built-in strategies keep M15 as their primary decision timeframe.
3. The fast watchdog still runs independently between AI decisions for trailing stops, break-even management, weekend protection, and spread-spike entry blocking.
...
3. Run this command to install all required packages:
   ```
   pip install python-dotenv anthropic openai requests pandas MetaTrader5 flask tzdata
   ```
...
| **Scalping** | ⚡ | Quick in-and-out trades targeting small, frequent profits with tight SL (20-50 pts) and TP (30-80 pts). Best during London/NY overlap. |
...
The bot ships with **4 built-in trading strategies**. Each strategy shapes the AI's decision-making by giving it a different system prompt with specific rules, entry/exit criteria, and risk parameters.
...
Quick in-and-out trades targeting small, frequent profits. The bot looks for short-term momentum on M5 candles, uses M15 as secondary structure confirmation, and keeps tight stop losses (20-50 points) with take profits (30-80 points). Best during London/NY overlap when liquidity is highest. High win rate, small gains per trade.
...
1. In the web dashboard, use the **Strategy** dropdown in the Control Panel
...
2. **Choose your Trading Strategy** — Select a strategy from the dropdown: Volatility Breakout, Scalping, Smart Money (ICT), or Conservative. Each shapes how the AI analyzes the market.
...
3. **Choose your Trading Sessions** — Use the session checkboxes to decide which market windows can open new trades. You can select one session or multiple sessions.

4. **Optional: Set Safety Override Checkboxes** — If you want the bot to keep evaluating trades even when a local pre-AI blocker would normally force HOLD, stop the bot first and enable the specific override checkbox you want. These overrides only bypass local pre-AI filters. They do **not** disable execution-time risk controls such as lot validation, spread enforcement during order placement, or account-aware trade rejection.

5. **Click "Start Trading"** — The bot will connect to MetaTrader 5, start fetching market data, and begin its 15-minute trading cycle.
...
|---------|---------------|
| **Control Panel** (left) | AI engine selector, strategy selector, trading session checkboxes, safety override checkboxes, account mode, start/stop buttons, close-all panic button, Telegram toggle, session status, daily P&L, and system info |
...
| **AI Decision** (top right) | The AI's latest trading decision — BUY (green), SELL (red), or HOLD (blue) — plus confidence level, lot size, stop loss, take profit, and the AI's reasoning |
...
Step 3 → Market status + Session filter
         • Check if the market is open (MT5 symbol trade_mode)
         • Skip trading entirely if market is closed (weekends, holidays)
...
Step 4 → Fetch multi-timeframe market data from MetaTrader 5
         • Primary intraday candles from the selected strategy cadence (M5 for scalping, M15 otherwise)
...
Step 6 → Centralized risk gate + AI analysis
         ...
         • Sends a compact payload: summarized candles, top news items, positions, recent decision memory, session context, calendar context, and headline-blackout context
         • Includes recent AI decisions + outcomes (AI memory)
...
The AI receives a strict system prompt (based on the selected trading strategy) forcing it to act as a quantitative analyst and output a structured JSON decision with: action (BUY/SELL/HOLD/CLOSE_BUY/CLOSE_SELL), confidence (LOW/MEDIUM/HIGH), lot size, stop loss, take profit, and reasoning. The bot uses a robust 3-tier JSON parser to extract the decision even if the AI wraps it in extra text.
...
| **TRADE_COOLDOWN** | 2 cycles | Minimum 2 cycles (30 min) between consecutive trades |
...
| **TRADE MEMORY** | Automatic | Learns from executed trades in SQLite, can block repeated weak setups, and stores a short reflection after profit reconciliation |
| **COMPACT AI PAYLOAD** | Automatic | Sends summarized market/news/history data to reduce token usage per AI request |
| **MARKET CLOSED DETECTION** | Auto | Checks MT5 symbol trade_mode — skips trading entirely when market is closed (weekends/holidays) |
...
- If this persists, check that your market data is being fetched correctly (look at the logs)
...
- You need to install the dependencies. Run:
  ```
  pip install python-dotenv anthropic openai requests pandas MetaTrader5 flask
  ```
...
│   └── strategies.py          ← 4 trading strategy definitions + AI system prompts
├── data/
...
├── execution/
│   ├── order_manager.py       ← Risk clamping, order execution, trailing/break-even stops
│   └── session_filter.py      ← Session-aware trading + market-closed detection + news blackout filter
├── notifications/
│   ├── __init__.py
